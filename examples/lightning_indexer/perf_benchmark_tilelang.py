#!/usr/bin/env python3
"""
lightning_indexer TileLang 算子性能基准采集脚本

用法:
  msprof op --kernel-name="main" --application="python3 perf_benchmark_tilelang.py --case=BSND_BSND" \
    --output=./perf_output_tilelang --aic-metrics=Default --launch-count=20 --warm-up=5

支持四种 Layout 场景，通过命令行参数选择:
  python3 perf_benchmark_tilelang.py --case=BSND_BSND
  python3 perf_benchmark_tilelang.py --case=BSND_PA_BSND
  python3 perf_benchmark_tilelang.py --case=TND_TND
  python3 perf_benchmark_tilelang.py --case=TND_PA_BSND
  python3 perf_benchmark_tilelang.py --case=ALL   # 跑全部四种
"""
...
# Add example dir to path for lightning_indexer import
sys.path.insert(0, "/home/tilelang-ascend/examples/lightning_indexer")
...
def build_case_params(case_name):
    """四种 Layout 代表性用例参数 — 与 AscendC 基准完全一致"""
    cases = {
        "BSND_BSND": {
...
def run_benchmark(params, num_iters=30):
    """运行 TileLang 性能基准"""
    print(f"\n{'='*80}")
    print(f"Case: {params['layout_query']}_{params['layout_key']} (TileLang)")
    ...
    print("  Compiling kernel (JIT)...")
    _ = tl_lightning_indexer(query, key, weights, **kwargs)
    ...
    for i in range(num_iters):
        result = tl_lightning_indexer(query, key, weights, **kwargs)
    end_event.record()
    torch.npu.synchronize()
...
def main():
    parser = argparse.ArgumentParser(description="lightning_indexer TileLang perf benchmark")
    ...
    print(f"\n{'='*80}")
    print("Benchmark Summary (TileLang lightning_indexer)")
    print(f"{'='*80}")
    ...
    import json
    with open("/home/tilelang-ascend/examples/lightning_indexer/perf_tilelang.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to: examples/lightning_indexer/perf_tilelang.json")
