...
- **v7.1 UB 容量 (Kernel 2 sort 缓存优化)**: sorted_cache(16KB) + sort_tmp(4KB) + merge_output(16KB) + global_topk(16KB) = 52KB + 其他 buffer < 192KB ✓（§8.8, v7.1 1D 平铺方案）
                ...
                if s2_start < act_k_batch:
                    ...
                    for s2_l1 in T.serial(T.ceildiv(BLOCK_N, S2_L1)):
                        ...
                        for m_l1 in T.serial(T.ceildiv(BLOCK_M, M_L1)):
                            ...
                            # v7.1: 4 个子 GEMM (独立 buffer, 无 L1 切片, transpose_B=True 精度正确)
...
| **v7.1 总计** | **52KB** | / 192KB UB (v7 原方案 48KB, +4KB sort_tmp) |
...
sorted_cache = T.alloc_ub([SORT_CACHE_SIZE * SORT_BUF_SIZE], calc_dtype)  # [4×1024] = 16KB (1D)
sort_tmp = T.alloc_ub([SORT_BUF_SIZE], calc_dtype)                       # [1024] = 4KB (sort dst, 完整 Buffer)
merge_output = T.alloc_ub([SORT_CACHE_SIZE * SORT_BUF_SIZE], calc_dtype) # [4×1024] = 16KB
...
for s2_block in T.serial(s2BaseNum):
    ...
    if is_cache_full or is_s2_end:
        ...
        if s2_block < SORT_CACHE_SIZE:
            ...
            T.copy(merge_output, global_topk_ub)
        else:
            # 后续块: global_topk_ub + merge_output → global_topk_ub (2-way merge, 取前 BASE_TOPK)
            T.tile.merge_sort(global_topk_ub, global_topk_ub, merge_output)
...
| **v7 sort 缓存优化** (v7 新增, v7.1 修正) | ✅ v7.1 修正完成 (验证 1+2) | SortedBasicBlock 4-way merge. **v7.1: 1D 平铺+sort_tmp+T.copy+切片 src 方案修正 sort 切片 dst 不可行 + way 参数不存在 (验证 1+2 PASS)** |
