...
| Q_L1 buffer | 2 bufs × [512,128]×2B = 256KB | **2 bufs × [256,128]×2B = 128KB** | -50% L1 占用 |
| K_L1 buffer | 3 bufs × [512,128]×2B = 384KB | **3 bufs × [256,128]×2B = 192KB** | -50% L1 占用 |
| L1 总占用 | 256KB + 384KB = 640KB > 512KB ⚠️ | **128KB + 192KB = 320KB < 512KB ✓** | v6 已超 L1 容量! v7 修正 |
| S1 外层循环 | 1 次（M_L1=512 覆盖全部） | **2 次**（M_L1=256，分 2 轮） | 增加循环次数但提升流水效率 |

> **⚠️ v6 L1 容量隐患**：v6 的 Q_L1(256KB) + K_L1(384KB) = 640KB > 512KB L1 容量！这是 v6 Cube 慢的潜在原因之一——L1 超限导致数据被驱逐到 GM，增加 MTE2 延迟。v7 对齐 AscendC 后 L1 总占用 320KB < 512KB ✓。

**v7 Cube 3 层 tiling 伪代码（对齐 AscendC `ComputeMm1` 结构）**：

```python
# v7: Cube 3 层 tiling (L1 → L0 → MMA), 对齐 AscendC service_cube.h:148-204
...
# v7: L1 buffer (对齐 AscendC buffer 大小)
Q_L1 = T.alloc_shared((M_L1, D), input_dtype)       # [256, 128] = 64KB (fp16)
K_L1 = T.alloc_shared((S2_L1, D), input_dtype)       # [256, 128] = 64KB (fp16)
C_L0 = T.alloc_fragment((M_L0, N_L0), calc_dtype)    # [128, 128] = 64KB (fp32)

# v7: 3 层循环 (对齐 AscendC ComputeMm1 的 s2GmOffset → s1gGmOffset → s2L1Offset → s1gL1Offset)
for s2_l1 in T.serial(T.ceildiv(s2ProcessSize, S2_L1)):   # L1 层 S2 循环 (AscendC: s2GmOffset)
    ...
    # K_L1: GM → L1 (Nd2Nz, 对齐 AscendC KeyNd2Nz)
    T.copy(Key[s2_l1 * S2_L1 : s2_l1 * S2_L1 + s2_l1_real, ...], K_L1)
    ...
    for m_l1 in T.serial(T.ceildiv(s1gProcessSize, M_L1)):  # L1 层 M 循环 (AscendC: s1gGmOffset)
        ...
        # Q_L1: GM → L1 (Nd2Nz, 对齐 AscendC QueryNd2Nz)
        T.copy(Query[m_l1 * M_L1 : m_l1 * M_L1 + m_l1_real, ...], Q_L1)

        # L0 层内循环 (对齐 AscendC s2L1Offset → s1gL1Offset)
        for s2_l0 in T.serial(T.ceildiv(s2_l1_real, N_L0)):  # L0 层 S2 循环
            s2_l0_real = T.min(N_L0, s2_l1_real - s2_l0 * N_L0)
            for m_l0 in T.serial(T.ceildiv(m_l1_real, M_L0)):  # L0 层 M 循环
                m_l0_real = T.min(M_L0, m_l1_real - m_l0 * M_L0)
                # T.gemm_v0: Q_L1 切片 × K_L1 切片 → C_L0
                T.gemm_v0(
                    Q_L1[m_l0 * M_L0 : m_l0 * M_L0 + m_l0_real, :],
                    K_L1[s2_l0 * N_L0 : s2_l0 * N_L0 + s2_l0_real, :],
                    C_L0,
                    transpose_B=True,
                    init=(s2_l0 == 0)  # N 维首次 init, 后续累加
                )
            # C_L0 完成一整行 [M_L0, s2_l0_real], copy 到 GM workspace (enable_relu)
            T.copy(C_L0,
                   qk_workspace[cid, s2_block % num_stages,
                                m_l1 * M_L1 + m_l0 * M_L0 : m_l1 * M_L1 + m_l0 * M_L0 + m_l0_real,
                                s2_l1 * S2_L1 + s2_l0 * N_L0 : s2_l1 * S2_L1 + s2_l0 * N_L0 + s2_l0_real],
                   enable_relu=True)
```
...
| Buffer | v6 大小 | v7 大小 | L1 容量 512KB |
|--------|---------|---------|--------------|
| Q_L1 (2 bufs) | 2 × [512,128]×2B = 256KB | **2 × [256,128]×2B = 128KB** | ✓ |
| K_L1 (3 bufs) | 3 × [512,128]×2B = 384KB | **3 × [256,128]×2B = 192KB** | ✓ |
| **总计** | **640KB > 512KB ❌** | **320KB < 512KB ✓** | v7 修正 v6 L1 超限 |

> **v7 关键发现**：v6 的 Q_L1 + K_L1 = 640KB 已超 L1 容量 512KB，这是 msprof 实测 Cube 慢 64% 的潜在原因。v7 对齐 AscendC 的 M_L1=256, S2_L1=256 后，L1 总占用 320KB < 512KB ✓。
...
- **v7 L1 容量** (v7 修正): Q_L1(2×64KB) + K_L1(3×64KB) = **320KB < 512KB ✓**（v6 的 640KB > 512KB 超限已修正, §5.2.1）
...
- **v7 UB 容量 (Kernel 2 sort 缓存优化)**: sorted_cache(48KB) + 其他 buffer < 192KB ✓（§8.8）
...
### 8.5 Cube/Vector 核计算流程（v7: Cube 3 层 tiling 对齐 AscendC + sort 缓存优化 + v6 API 优化保留）

> **v7 关键更新**：
> 1. **Cube 侧**：M_L1=256, S2_L1=256 对齐 AscendC（v6 的 M_L1=512 导致 L1 超限 640KB > 512KB），3 层 tiling (L1→L0→MMA)
> 2. **Vector 侧**：保留 v6 的 row_expand_mul + reduce_sum(clear=False) + pipe_barrier("v")，新增 sort 缓存优化（§8.8 详述）
...
with T.Kernel(launch_core_num, threads=2, is_npu=True) as (cid, vid):
    # --- UB/L1/L0C buffer 在 pipeline body 外分配 (避免 ring-buffer, Q19) ---
    # v7: L1 buffer 对齐 AscendC (M_L1=256, S2_L1=256), 修正 v6 L1 超限 (640KB→320KB)
    Q_L1 = T.alloc_shared((M_L1, D), input_dtype)       # v7: L1, [256,128]=64KB (v6: [512,128]=128KB)
    K_L1 = T.alloc_shared((S2_L1, D), input_dtype)       # v7: L1, [256,128]=64KB (v6: [512,128]=128KB)
        ...
        if task_id < total_tasks:
            ...
            for s2_block in T.Pipelined(s2BaseNum, num_stages=num_stages, cross_interval=2):
                ...
                if s2_start < act_k_batch:
                    ...
                    # v7: Cube 3 层 tiling GEMM (对齐 AscendC ComputeMm1 结构, §5.2.1 详述)
                    # v6 的 L0 inner split 升级为 v7 的 3 层 tiling (L1→L0→MMA)
                    # Q_L1=[M_L1=256, D=128], K_L1=[S2_L1=256, D=128] 在 L1
                    # C_L0=[M_L0=128, N_L0=128] 在 L0C
                    # T.gemm_v0 的 M,N 从 C_L0 shape 推导 (ascend.py:413)

                    # v7: L1 层 S2 循环 (AscendC: s2GmOffset, S2_BASIC_BLOCK=256)
                    for s2_l1 in T.serial(T.ceildiv(BLOCK_N, S2_L1)):
                        s2_l1_real = T.min(S2_L1, BLOCK_N - s2_l1 * S2_L1)
                        # K_L1: GM → L1 (仅 S2_L1 行, 对齐 AscendC KeyNd2Nz)
                        T.copy(Key[... s2_l1 * S2_L1 : s2_l1 * S2_L1 + s2_l1_real ...], K_L1)

                        # v7: L1 层 M 循环 (AscendC: s1gGmOffset, M_BASIC_BLOCK=256)
                        for m_l1 in T.serial(T.ceildiv(BLOCK_M, M_L1)):
                            m_l1_real = T.min(M_L1, BLOCK_M - m_l1 * M_L1)
                            # Q_L1: GM → L1 (仅 M_L1 行, 对齐 AscendC QueryNd2Nz)
                            T.copy(Query[... m_l1 * M_L1 : m_l1 * M_L1 + m_l1_real ...], Q_L1)

                            # v7: L0 层内循环 (对齐 AscendC s2L1Offset → s1gL1Offset)
                            for s2_l0 in T.serial(T.ceildiv(s2_l1_real, N_L0)):
                                s2_l0_real = T.min(N_L0, s2_l1_real - s2_l0 * N_L0)
                                for m_l0 in T.serial(T.ceildiv(m_l1_real, M_L0)):
                                    m_l0_real = T.min(M_L0, m_l1_real - m_l0 * M_L0)
                                    # T.gemm_v0 + BufferRegion 切片 (Q21)
                                    T.gemm_v0(
                                        Q_L1[m_l0 * M_L0 : m_l0 * M_L0 + m_l0_real, :],  # L1 切片 [M_L0, D]
                                        K_L1[s2_l0 * N_L0 : s2_l0 * N_L0 + s2_l0_real, :],  # L1 切片 [N_L0, D]
                                        C_L0,                                              # [M_L0, N_L0]
                                        transpose_B=True,
                                        init=(s2_l0 == 0)                                   # N 维首次 init, 后续累加
                                    )
...
| Buffer | 大小 | 用途 |
|--------|------|------|
| `sorted_cache_ub` | [4, S2_VEC_BLOCK, 2] × 4B = 4 × 512 × 2 × 4 = **16KB** | 缓存 4 个 S2 块的 sort 结果 (value, index 对) |
| `global_topk_ub` | [BASE_TOPK, 2] × 4B = 2048 × 2 × 4 = **16KB** | 全局 topk 结果 (跨 S2 块归并) |
| `merge_tmp_ub` | [BASE_TOPK, 2] × 4B = **16KB** | 4-way merge 临时 buffer |
| **v7 新增总计** | **48KB** | / 192KB UB |

> **v7 UB 容量验证**：v6 Kernel 1 UB=34KB + v7 sort 缓存 48KB = 82KB < 192KB ✓（余量 110KB）
>
> **注意**：sort 缓存优化在 **Kernel 2**（topk）中实施，不影响 Kernel 1 的 UB 占用。Kernel 2 的 UB 预算独立于 Kernel 1。

**v7 sort 缓存优化伪代码（Kernel 2 内）**：

```python
# v7: sort 缓存优化 (对齐 AscendC SortedBasicBlock 机制)
...
# v7: UB buffer (Kernel 2 内, 独立于 Kernel 1)
sorted_cache_ub = T.alloc_shared((SORT_CACHE_SIZE, S2_VEC_BLOCK, 2), calc_dtype)  # [4, 512, 2] = 16KB
global_topk_ub = T.alloc_shared((BASE_TOPK, 2), calc_dtype)                       # [2048, 2] = 16KB
merge_tmp_ub = T.alloc_shared((BASE_TOPK, 2), calc_dtype)                         # [2048, 2] = 16KB
...
for s2_block in T.serial(s2BaseNum):
    ...
    # 3. sort 当前 S2 块 (对齐 AscendC Sort<float, true>)
    # 结果写入 sorted_cache_ub[cache_idx]
    T.tile.sort(sorted_cache_ub[cache_idx], score_buf, S2_VEC_BLOCK)

    cache_idx += 1

    # 4. 缓存满 4 块或 S2 结束时, 进行 4-way merge (对齐 AscendC MrgBasicBlock)
    ...
    if is_cache_full or is_s2_end:
        if s2_block < SORT_CACHE_SIZE:
            # 前 4 块: 直接 4-way merge 到 global_topk_ub
            # 对齐 AscendC: MrgBasicBlock(globalTopkUb_, tt, cache_idx, s2BaseSize_)
            T.tile.merge_sort(
                global_topk_ub,                              # dst: 全局 topk
                sorted_cache_ub[0],                          # src1
                sorted_cache_ub[1],                          # src2
                sorted_cache_ub[2],                          # src3
                sorted_cache_ub[3],                          # src4 (cache_idx < 4 时部分有效)
                way=cache_idx                                # 实际有效路数
            )
        else:
            # 后续块: 先 4-way merge 到 merge_tmp_ub, 再与 global_topk_ub 归并
            T.tile.merge_sort(
                merge_tmp_ub,
                sorted_cache_ub[0],
                sorted_cache_ub[1],
            ...
            # global_topk_ub + merge_tmp_ub → global_topk_ub (取前 BASE_TOPK)
            T.tile.merge_sort(global_topk_ub, global_topk_ub, merge_tmp_ub, way=2)

        cache_idx = 0  # 重置缓存

# 5. 最终 topk: 从 global_topk_ub 提取前 sparse_count 个
...
**v7 sort 缓存优化的 API 验证**：

| API | 源码位置 | 使用验证 | 说明 |
|-----|---------|---------|------|
| `T.tile.sort` | `ascend_tile.py` | `examples/sort/example_sort.py` | 单块排序 |
| `T.tile.merge_sort` | `ascend_tile.py:320` | `examples/sort/example_merge_sort.py` | 2/3/4-way merge，支持 `way` 参数指定有效路数 |

> **⚠️ v7 Stage 2 验证项**：
> 1. `T.tile.merge_sort` 的 4-way merge 是否支持 `way=4` 参数（当前文档仅明确 2/3/4-way）
> 2. `sorted_cache_ub[cache_idx]` 的 3D buffer 切片是否被 T.tile.sort 支持
> 3. 缓存不满 4 块时的 `way=cache_idx`（如 way=2, way=3）是否正确处理
> 4. fallback：若 4-way merge 不可用，退回 2-way merge（2 次 2-way merge 替代 1 次 4-way）
...
| **v7 Cube 3 层 tiling** (v7 新增) | ⚠️ 需 Stage 2 验证 | M_L1=256, S2_L1=256 对齐 AscendC. ⚠️ 验证 3 层嵌套循环编译通过, L1 容量 320KB < 512KB ✓ |
| **v7 sort 缓存优化** (v7 新增) | ⚠️ 需 Stage 2 验证 | SortedBasicBlock 4-way merge. ⚠️ 验证 T.tile.merge_sort 4-way + way 参数, 3D buffer 切片 |
| **v7 TND_PA_BSND block_size=16** (v7 新增) | 🔴 需排查修复 | msprof 实测超时. ⚠️ 排查 block_size=16 时 tiling/buffer/sync 问题 |
...
- **v7 3 层 tiling 结构**（对齐 AscendC `service_cube.h:148-204`）：
  ...
  L1 层 S2 循环 (S2_L1=256)        ← AscendC: s2GmOffset
    ...
    └─ L1 层 M 循环 (M_L1=256)     ← AscendC: s1gGmOffset
         └─ Q_L1: GM → L1 (Nd2Nz)
         └─ L0 层 S2 循环 (N_L0=128)  ← AscendC: s2L1Offset
              └─ L0 层 M 循环 (M_L0=128) ← AscendC: s1gL1Offset
                   └─ T.gemm_v0(Q_L1 切片, K_L1 切片, C_L0)
...
- **v6 vs v7 对比**：
  ...
  | 参数 | v6 | v7 | 变化 |
  |------|----|----|------|
  | tiling 层数 | 2 层 (L0 inner split) | **3 层 (L1→L0→MMA)** | 对齐 AscendC |
  | M_L1 | S1_BLOCK×G=512 | **256** (对齐 AscendC) | -50% |
  | S2_L1 | s2BaseSize=512 | **256** (对齐 AscendC) | -50% |
  | Q_L1 + K_L1 | 640KB > 512KB ❌ | **320KB < 512KB ✓** | 修正 L1 超限 |
  | S1 外层循环 | 1 次 | **2 次** (M_L1=256) | 增加循环但提升流水 |

- **L1 容量验证**：
  - v6: Q_L1(2×128KB) + K_L1(3×128KB) = 256KB + 384KB = **640KB > 512KB ❌**
  - v7: Q_L1(2×64KB) + K_L1(3×64KB) = 128KB + 192KB = **320KB < 512KB ✓**

- **预期收益**：BSND_BSND Cube 时间 98.83us → ~65-70us（-30~35%），主要来自 L1 容量修正和流水效率提升

- **⚠️ Stage 2 验证项**：
  1. 3 层嵌套循环（s2_l1 → m_l1 → s2_l0 → m_l0）编译通过
  2. `T.copy(Query[m_l1 * M_L1 : ..., ...], Q_L1)` 部分行加载（非全量加载）编译通过
  3. `T.copy(C_L0, qk_workspace[..., m_slice, s2_slice, :], enable_relu=True)` 2D 部分写入编译通过
...
- **v7 修复方案**：
  1. **排查 PA 寻址**：block_size=16 时，检查 `T.copy(Key[block_table[b, s2_block], :, 0, :], K_L1)` 是否因 block_size 过小导致 DMA 效率低
  2. **PA 寻址优化**：block_size=16 时，考虑按多个 block 聚合加载（如一次加载 8 个 block = 128 行到 K_L1）
  3. **BF16 路径验证**：确认 BF16 的 GEMM（T.gemm_v0）和 Vector（row_expand_mul, reduce_sum）路径正常
  4. **TND+PA 交互**：检查 TND query 的 act_seq 前缀和读取与 PA key 的 block_table 寻址是否正确交互

- **⚠️ Stage 2 验证项**：
  1. 单独运行 TND_PA_BSND 用例，确认是否复现超时
  2. 尝试 block_size=32/64/128 的 TND_PA_BSND，确认是否 block_size=16 特有问题
  3. 检查 PA 寻址的 DMA 效率（msprof MemoryUB.csv）
...
| **v7: 4-way merge_sort 不可用** (v7 风险) | T.tile.merge_sort 不支持 way=4 或 3D buffer 切片 | 编译报错 | fallback: 退回 2-way merge (2 次 2-way merge 替代 1 次 4-way), 或退回 v6 标准 sort+merge_sort 路径 |
| **v7: TND_PA_BSND block_size=16 超时** (v7 新增) | block_size=16 时 PA 寻址碎片化 | 运行超时 | 排查 PA 寻址 DMA 效率, 考虑多 block 聚合加载 (§10.2 Q27) |
| **v7: 3 层 tiling 循环编译失败** (v7 风险) | 4 层嵌套循环 (s2_l1→m_l1→s2_l0→m_l0) 编译器不支持 | 编译报错 | fallback: 退回 v6 的 2 层 L0 inner split (M_L0, N_L0), 但 Q_L1/K_L1 改为 [M_L1=256, D] (仅修正 L1 容量) |
...
| `lightning_indexer.py` | ✅ 已实现 | 双 Kernel: lightning_indexer_score + lightning_indexer_topk + wrapper (v6 实现, v7 待优化) |
...
2. ✅ kernel 实现 (`lightning_indexer.py`, 双 Kernel @tilelang.jit + wrapper) — v6 已实现
...
| P0 | sort 缓存优化 — S1≤4 时缓存 4 块再 4-way merge | merge_sort 次数 75% | BSND_BSND, TND_TND | §8.8 |
...
### 12.7 v7 优化的 Stage 2 验证清单

**v7 新增项**（Cube 3 层 tiling + sort 缓存 + TND_PA_BSND 修复）：
- [ ] **v7: Cube 3 层 tiling** — M_L1=256, S2_L1=256, 4 层嵌套循环编译通过（§5.2.1）
- [ ] **v7: L1 容量验证** — Q_L1(128KB) + K_L1(192KB) = 320KB < 512KB（get_kernel_source 验证）
- [ ] **v7: T.copy(Query[m_l1*M_L1:..., ...], Q_L1)** — 部分行加载编译通过
- [ ] **v7: T.copy(C_L0, qk_workspace[..., m_slice, s2_slice, :], enable_relu=True)** — 2D 部分写入编译通过
- [ ] **v7: sort 缓存优化** — SortedBasicBlock 4-way merge 编译通过（§8.8）
- [ ] **v7: T.tile.merge_sort 4-way** — way=4 参数验证
- [ ] **v7: sorted_cache_ub[cache_idx] 3D 切片** — T.tile.sort 3D buffer 切片验证
- [ ] **v7: sort 缓存 UB 占用** — 48KB < 192KB（get_kernel_source 验证）
- [ ] **v7: TND_PA_BSND block_size=16** — 超时问题修复（§10.2 Q27）
- [ ] **v7: 4 个 msprof 基准场景性能达标** — ≤ 80% 目标（4/4 达标）
- [ ] **v7 fallback**: 若 3 层 tiling 编译失败, 退回 v6 2 层 L0 inner split + Q_L1/K_L1 改为 [M_L1=256, D]
- [ ] **v7 fallback**: 若 4-way merge 不可用, 退回 2-way merge 或 v6 标准 sort+merge_sort 路径
