...
def make_lightning_indexer_kernel(
    ...
    # Adjust core_num: when tasks_per_core == num_s2_blocks, try reducing core_num
    # so each core handles multiple BSNs (better pipeline depth).
    # Note: s2_per_group was computed but never used in kernel; removed.
                    ...
                    for _real_off in T.serial(_real_count):
                        ...
                        for s1_local in range(VID_S1):
                            ...
                            if s1_start + actual_s1 < _q_len_b and s2_start < s2_valid:
                                ...
                                # Note: only handles VECTOR_BASEG in {8,4,2}. VECTOR_BASEG==1
                                # would skip this block but then line 770 add(reduce_g_ub,
                                # reduce_tmp_ub[0,:], reduce_tmp_ub[1,:]) would OOB. Currently
                                # safe because G>=8 always selects VECTOR_BASEG=8.
                                if VECTOR_BASEG == 8:
...
def lightning_indexer(
    ...
    *,
    ...
    return_value: bool = False,
    # Internal tuning params (not part of official API)
    block_n: Optional[int] = None,
    pp_slots: int = 2,
    max_cores: Optional[int] = None,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """TileLang lightning_indexer -- compatible with torch_npu.npu_lightning_indexer.

    Interface spec:
      /home/ops-transformer/attention/lightning_indexer/tests/pytest/li.md
    ...
    if is_tnd:
        ...
        N2 = key.shape[2] if is_pa else (key.shape[1] if is_tnd_key else key.shape[2])
    else:
    ...
    # ==== Normalize actual_seq_lengths ====
    # Official API: TND uses prefix-sum, BSND/PA_BSND uses per-batch length
    if is_tnd:
        assert actual_seq_lengths_query is not None, "TND query requires actual_seq_lengths_query"
        asq = actual_seq_lengths_query.to(device=query.device, dtype=torch.int32)
        ...
        q_offset = torch.zeros(B, dtype=torch.int32, device=query.device)
        actual_q_len = actual_seq_lengths_query

    if is_tnd_key:
        assert actual_seq_lengths_key is not None, "TND key requires actual_seq_lengths_key"
        ask = actual_seq_lengths_key.to(device=query.device, dtype=torch.int32)
        k_lens = torch.zeros(B, dtype=torch.int32, device=query.device)
        k_lens[0] = ask[0]
        k_lens[1:] = ask[1:] - ask[:-1]
        k_offset = torch.zeros(B, dtype=torch.int32, device=query.device)
        k_offset[1:] = ask[:-1]
        S2 = int(k_lens.max().item())
        S2 = max(S2, 257)
        actual_k_len = k_lens
    elif is_pa:
        assert actual_seq_lengths_key is not None, "PA_BSND key requires actual_seq_lengths_key"
        actual_seq_lengths_key = actual_seq_lengths_key.to(device=query.device, dtype=torch.int32)
        k_offset = torch.zeros(B, dtype=torch.int32, device=query.device)
        S2 = int(actual_seq_lengths_key.max().item())
        S2 = max(S2, 257)
        actual_k_len = actual_seq_lengths_key
    else:
    ...
    # Pad TND key to avoid OOB on tail blocks
    if is_tnd_key:
        _s2_eff = max(S2, 257)
    ...
    # Pad block_table to (B, max_block_num)
    ...
    func = make_lightning_indexer_kernel(
        B=B, S1=S1, S2=S2, N1=N1, D=D, N2=N2,
        TOP_K=sparse_count, sparse_mode=sparse_mode,
        input_dtype=input_dtype, block_n=block_n, max_cores=max_cores,
        layout_query=layout_query, layout_key=layout_key,
        block_size=_block_size, max_block_num=max_block_num,
        q_t_size=q_tot if is_tnd else None,
        k_t_size=_k_tot if is_tnd_key else None,
        pp_slots=pp_slots, return_value=return_value,
    )
    if is_tnd_key:
        key = key.unsqueeze(0)
    if is_tnd:
    ...
    # Kernel receives per-batch lengths (not prefix-sum)
    func_out = func(
        query, key, weights,
        actual_q_len, actual_k_len,
        block_table, q_offset, k_offset,
    )
